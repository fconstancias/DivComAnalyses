...
```{r, message = FALSE}
vegan::adonis(bc ~ get_variable(physeq_1_rare, "treatment_invivo"))$aov.tab
```
...
Since it is significant, we can run pairwise comparisons to further investigate:
...
adding taxa correlated with ordination pattern:
...
physeq_1_rare %>% 
  plot_ordination(physeq = ., 
                  ...
                  label= NULL) + 
  theme_bw() +
  scale_color_manual(values = treat_pal) -> pca
...
```{r}
physeq_1 %>% 
  phyloseq_add_metadata_vector(dist = dm,
                               phyloseq = .,
                               figure_ord = out$plot,
...
pcoa_group %>% 
  phyloseq_plot_ordinations_facet(plot_list = ., return_eig_df = TRUE, color_group = "day")
```
db-RDA:
