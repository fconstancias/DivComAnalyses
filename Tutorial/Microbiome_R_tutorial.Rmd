...
Install R/ Rstudio from the ETH AppV Kiosk or following the instructions [here](https://rstudio-education.github.io/hopr/starting.html)
...
`speedyseq` can be used to speed the process:

```{r, warning=FALSE, message=FALSE}
physeq_1_st %>% 
  physeq_glom_rename(taxrank = "Strain", 
                     speedyseq = TRUE) %>% # uses speedyseq R package which significantly speeds up some steps.
  tax_table() %>% 
  head()
```
