...
#' as.data.frame() -> data
#'
#' phyloseq_filter_samples(GlobalPatterns, 50) -> results
#'
...
phyloseq_filter_samples <- function(physeq, thrs)
{
  ...
  for (S in samples)
  {
    ...
    df %>%
      ...
      dplyr::select(ASV, (!!S)) %>%
      arrange(-get(S)) %>%
      mutate(percent = get(S)/sum(get(S))) %>%
      mutate(Accumulated_pc = cumsum(percent)) %>%
      mutate(Common_Rare = ifelse(Accumulated_pc <= thrs/100, "Common", "Rare")) -> tpm

    tpm %>%
      mutate(new_count = ifelse(Common_Rare == "Common", get(S), 0)) %>%
      select(- percent, - Accumulated_pc, -!!S, -Common_Rare) %>%
      dplyr::rename(!!S:= new_count) -> filtter_sample_common

    tpm %>%
      mutate(new_count = ifelse(Common_Rare == "Rare", get(S), 0)) %>%
      select(- percent, - Accumulated_pc, -!!S, -Common_Rare) %>%
      dplyr::rename(!!S:= new_count) -> filtter_sample_rare

    tpm %>%
      select(- percent, - Accumulated_pc, -!!S) %>%
      dplyr::rename(!!S:= Common_Rare) -> results
